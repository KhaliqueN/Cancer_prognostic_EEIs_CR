...
# ##------ plot for the cancer-relevant edges --------
# ctd <- data.frame(cancer=cancer_type, value=contact_pert)
# ctd$FLAG <- rep('Contact-based', length(cancer_type))
# ptd <- data.frame(cancer=cancer_type, value=pisa_pert)
# ptd$FLAG <- rep('Energy-based', length(cancer_type))
# etd <- data.frame(cancer=cancer_type, value=eppic_pert)
# etd$FLAG <- rep('Evolution-based', length(cancer_type))

# td <- rbind(ctd, ptd)
# pdata <- rbind(td, etd)
# pdata$value <- pdata$value*100

# basesize  <- 8
# ppx <- ggplot(data = pdata, aes(x = cancer, y=value, fill=FLAG)) + geom_bar(stat="identity", position="dodge")+
# xlab("Cancer type")+
# scale_fill_manual(values=c('#e41a1c','#377eb8','#4daf4a'))+
# ylab("% of edges of a global EEIN with\n significant cancer survival correlation")+
...
#     axis.line = element_line(colour = "black"), axis.title = element_text(size=1*basesize), legend.text=element_text(size=1*basesize))+
# guides(fill=guide_legend(title="Approach"))

# ggsave(ppx,filename=paste0("../results/Final_results/cancer_relevant_edges.png"),width=7, height=3, dpi=400)

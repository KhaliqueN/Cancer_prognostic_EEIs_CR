...
##-- no correction ----
##-- Compare the number of real survival edges with the number of random survival edges -------
...
in_dir <- paste0('../data/CONTACT_weighted_networks/thres_6_1')
net_file <- data.table::fread(paste0('../data/CONTACT_networks/CONTACT_net_6_1.txt'))
net_file$PDBID <- unlist(lapply(strsplit(net_file[[1]], '[_]'),'[[',2))
net_file <- cbind(net_file[,c(3,4)], net_file[,-c(3,4)])
p1 <- unlist(lapply(strsplit(net_file[[3]], '[_]'),'[[',1))
p2 <- unlist(lapply(strsplit(net_file[[4]], '[_]'),'[[',1))
net_fi <- cbind(net_file[,c(1,2,3,4)], data.frame(p1,p2))
net_file <- cbind(net_fi, net_file[,c(5,6,7,8,9,10,11,12)])

...
for(thr in 1:length(cpm_threshold)){
	...
	for(cn in 1:length(cancer_type)){
		...
		pisa_g <- data.table::fread(paste0('../results/PISA_survival/threshold_',cpm_threshold[thr],'/',cancer_type[cn],'_Gained_Surv_.txt'))
		pisa_g <- pisa_g[pisa_g$pval <= pval_thres, ][,c(1,2)]
		pisa_l <- data.table::fread(paste0('../results/PISA_survival/threshold_',cpm_threshold[thr],'/',cancer_type[cn],'_Lost_Surv_.txt'))
		pisa_l <- pisa_l[pisa_l$pval <= pval_thres, ][,c(1,2)]
		pisa <- igraph::graph_from_data_frame(rbind(pisa_g,pisa_l), directed=FALSE)

		eppic_g <- data.table::fread(paste0('../results/EPPIC_survival/threshold_',cpm_threshold[thr],'/',cancer_type[cn],'_Gained_Surv_.txt'))
		eppic_g <- eppic_g[eppic_g$pval <= pval_thres, ][,c(1,2)]
		eppic_l <- data.table::fread(paste0('../results/EPPIC_survival/threshold_',cpm_threshold[thr],'/',cancer_type[cn],'_Lost_Surv_.txt'))
		eppic_l <- eppic_l[eppic_l$pval <= pval_thres, ][,c(1,2)]
		eppic <- igraph::graph_from_data_frame(rbind(eppic_g,eppic_l), directed=FALSE)

		contact_g <- data.table::fread(paste0('../results/CONTACT_survival/threshold_',cpm_threshold[thr],'/',cancer_type[cn],'_Gained_Surv_.txt'))
		contact_g <- contact_g[contact_g$pval <= pval_thres, ][,c(1,2)]
		contact_l <- data.table::fread(paste0('../results/CONTACT_survival/threshold_',cpm_threshold[thr],'/',cancer_type[cn],'_Lost_Surv_.txt'))
...
strip.text = element_text(size = basesize * 0.8), axis.title=element_text(basesize * 0.8))+ 
guides(color=guide_legend(title="Edge type"))
ggsave(p,filename=paste0("../results/Final_results/NETLOW_EEI_CPM_variation_avg.png"),width=7, height=3.5, dpi=400)
...
for(thr in 1:length(cpm_threshold)){
	...
	for(cn in 1:length(cancer_type)){
		...
		allsurv <- igraph::union(igraph::union(igraph::intersection(pisa,eppic,keep.all.vertices=FALSE),
		igraph::intersection(contact,eppic,keep.all.vertices=FALSE)), 
		igraph::intersection(contact,pisa,keep.all.vertices=FALSE))
...
for(k in 1:length(cancer_type)){
	tempq <- pdata2[pdata2$Cancer == cancer_type[k], ]
	temp3 <- tempq[tempq$variable == 'Cancer relevant', ]
	maxv <- c(maxv, temp3$Threshold[which(temp3$value == max(temp3$value))[1]])
	temp3 <- tempq[tempq$variable == 'Perturbed', ]
	maxvp <- c(maxvp, temp3$Threshold[which(temp3$value == max(temp3$value))[1]])
}
thres_count_surv <- plyr::count(maxv)
thres_count_surv$flag <- rep('Cancer-relevant perturbed edges', length(thres_count_surv[[1]]))
...
strip.text = element_text(size = basesize * 0.8), axis.title=element_text(basesize * 0.8))+ 
guides(color=guide_legend(title="Edge type"))
ggsave(p,filename=paste0("../results/Final_results/NETMEDIUM_EEI_CPM_variation_avg.png"),width=7, height=3.5, dpi=400)
...
for(thr in 1:length(cpm_threshold)){
	...
	for(cn in 1:length(cancer_type)){
		...
		allsurv <- igraph::intersection(igraph::intersection(pisa,eppic,keep.all.vertices=FALSE),contact,keep.all.vertices=FALSE) 
...
strip.text = element_text(size = basesize * 0.8), axis.title=element_text(basesize * 0.8))+ 
guides(color=guide_legend(title="Edge type"))
ggsave(p,filename=paste0("../results/Final_results/NETHIGH_EEI_CPM_variation_avg.png"),width=5, height=2.5, dpi=400)

...
pval_thres <- c(0.5) ## Threshold to define an EEIN 
...
all_net_data <- list()
for(j in 1:length(pval_thres)){
	all_net_data[[j]] <- data.table::fread(paste0('../data/PISA_networks_filtered/PISA_EEIN_',pval_thres[j],'.txt'))
}
...
for(pp in 1:length(pval_thres)){
	...
	out_dir <- paste0('../data/PISA_weighted_networks/',pval_thres[pp])
	dir.create(out_dir, recursive=TRUE)

	net_file <- all_net_data[[pp]][,c(1,2)]
	nodes <- union(net_file[[1]], net_file[[2]])

	##-- remove the edges with no exon name --> some exons could be missed becasue of the PDB uniprot mapping or missed interface residues
	wh1 <- which(net_file[[1]] == '')
	wh2 <- which(net_file[[2]] == '')
	wh <- union(wh1, wh2)
	if(length(wh) > 0){
		net_file <- net_file[-wh,]
	}

...
c_type <- args[1]#'KICH'#
outdir3 <- args[2]#'../results/Final_survival/level1'#
level <- args[3]#'level1'#

cpm_threshold <- c(0.5) ## Threshold to define whether an exon is expressed or not
...
for(qq in 1:length(allnets)){
    ...
    in_dir <- paste0('../data/Final_weighted_networks/',level,'/',strsplit(basename(allnets[qq]),'[.]')[[1]][1])
    net_file <- data.table::fread(allnets[qq], header=FALSE)
    nodes <- union(net_file[[1]], net_file[[2]])

    outdir4 <- paste0(outdir3, '/', strsplit(basename(allnets[qq]),'[.]')[[1]][1])
    if(!dir.exists(outdir4)){
    dir.create(outdir4, recursive=TRUE)
    }

    ##--- save exclusive edges and plot the percentage of patients that have these edges
    for(thr in 1:length(cpm_threshold)){
        ...
        save_GL_EEI(only_gained,paste0('Gained_'), outdir4, cpm_threshold[thr], c_type, gained, net_file)
        save_GL_EEI(only_lost,paste0('Lost_'), outdir4, cpm_threshold[thr], c_type, lost, net_file)  
        ...
        save_selected_EEI(sgpv[[1]],paste0('Gained_Surv_'),outdir4,cpm_threshold[thr],c_type,sgpv[[2]],gained,net_file, fracg1_test, fracg2_test)
        save_selected_EEI(slpv[[1]],paste0('Lost_Surv_'),outdir4,cpm_threshold[thr],c_type,slpv[[2]],lost,net_file, fracl1_test, fracl2_test)

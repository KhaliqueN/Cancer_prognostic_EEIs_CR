...
store_dir <- '../data/CONTACTS'
if(!dir.exists(store_dir)){
	dir.create(store_dir)
}

cmx <- fread('../data/uniprot_pdb_Ensembl_final.txt', header=TRUE)
...
cmx_f <- cmx[wh,] 
upro <- union(cmx_f$PROTEIN1, cmx_f$PROTEIN2)
updb <- unique(cmx_f$ID)



# after all filtering 13,190 out of 13,240 filtered complexes remain
fwrite(cmx_f, '../data/uniprot_pdb_Ensembl_finalized.txt', sep='\t', row.names=FALSE, quote=FALSE)
####----------------------------------------------------------------------------------------------------
